...
1. Complete the intialization steps to setup variables.
   - If you are using the [K8s-101 workshop environment](https://fortinetcloudcse.github.io/k8s-101-workshop/03_participanttasks/03_01_k8sinstall/03_01_02_k8sinstall.html), you can continue in the K8s-101 environment and choose [Option 1](/01gettingstarted/4_task3.html#option-1-continue-from-k8s-101-session). 
   - If you are on the K8s-201 environment, choose [Option 2](/01gettingstarted/4_task3.html#option-2-create-self-managed-k8s) or [Option 3](/01gettingstarted/4_task3.html#option-3-create-aks) to start from K8s-201 directly.
...
Check status with `kubectl get node -o wide`
```
NAME                        STATUS   ROLES           AGE     VERSION   INTERNAL-IP   EXTERNAL-IP   OS-IMAGE             KERNEL-VERSION     CONTAINER-RUNTIME
...
you can use `az network public-ip list -o table` to find the public ip address of nodes.

for example, you can use ssh ubuntu@52.224.219.58 and ssh ubuntu@40.71.204.87 to ssh into both nodes.

```
k8s51 [ ~ ]$ az network public-ip list -o table
...
In cFOS egress use case, it rely on CNI to route traffic from application container to cFOS, therefore, it is important to understand the CNI you are used in your k8s cluster. 
